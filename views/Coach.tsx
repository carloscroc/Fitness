...
import { LazyExerciseDetailModal } from '../components/LazyModal.tsx';
import { getExerciseImage } from '../utils/images.ts';
          ...
          <div className="flex gap-2 overflow-x-auto no-scrollbar pb-1 -mx-6 px-6">
            {EXERCISE_CATEGORIES.map(cat => (
              <button
                ...
                className={`px-5 py-2.5 rounded-full text-[11px] font-bold uppercase tracking-wider transition-all border whitespace-nowrap ${libraryCategory === cat
        ...
        <motion.div variants={itemVariants} className="grid grid-cols-2 gap-4">
          {filteredExercises.map(ex => (
            <motion.div
              ...
              className="aspect-[3/4] relative rounded-[32px] overflow-hidden active:scale-[0.98] transition-transform cursor-pointer group shadow-lg bg-[#1C1C1E] border border-white/5"
            >
              <img
              ...
              <div className="absolute bottom-5 left-5 right-5">
                ...
                <div className="text-[17px] font-bold font-display text-white leading-[1.1]">{ex.name}</div>
              </div>
            </motion.div>
          ))}
        </motion.div>
          ...
          <div className="flex gap-8">
            {(['Library', 'Body', 'History'] as Tab[]).map((tab) => (
              <button
                ...
                className={`pb-2 text-[15px] font-bold transition-all relative whitespace-nowrap ${activeTab === tab ? 'text-white' : 'text-zinc-500 hover:text-zinc-300'
                  }`}
              >
                {tab}
                {activeTab === tab && (
